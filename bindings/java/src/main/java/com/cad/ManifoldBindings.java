...
public class ManifoldBindings {
	...
	// ===== Primitives =====
		...
		load("manifold_cube", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_INT);

		// ManifoldManifold* manifold_sphere(void* mem, double radius, int circular_segments);
		load("manifold_sphere", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_INT);

		// ManifoldManifold* manifold_cylinder(void* mem, double height, double radius_low, double radius_high, int circular_segments, int center);
		load("manifold_cylinder", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT);

	// ===== Boolean operations =====

		// ManifoldManifold* manifold_boolean(void* mem, ManifoldManifold* a, ManifoldManifold* b, ManifoldOpType op); OpType : char { Add, Subtract, Intersect };
		load("manifold_boolean",       ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_INT);
		...
		load("manifold_intersection",  ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS);

		// ManifoldManifold* manifold_batch_boolean(void* mem, ManifoldManifoldVec* ms, ManifoldOpType op);
		load("manifold_batch_boolean", ValueLayout.ADDRESS,	ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_INT);
	...
	// ===== Transforms =====
		...
		// ManifoldManifold* manifold_transform(void* mem, ManifoldManifold* m, double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3, double z3, double x4, double y4, double z4);
		load("manifold_transform", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, // men, m,
			...
			ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE); // row 4 (x4,y4,z4) - translation

		// ManifoldManifold* manifold_translate(void* mem, ManifoldManifold* m, double x, double y, double z);
		load("manifold_translate", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifold* manifold_scale(void* mem, ManifoldManifold* m, double x, double y, double z);
		load("manifold_scale", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifold* manifold_mirror(void* mem, ManifoldManifold* m, double nx, double ny, double nz);
		load("manifold_mirror", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifold* manifold_rotate(void* mem, ManifoldManifold* m, double x, double y, double z);
		load("manifold_rotate", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE);

	// ===== Refinement =====

		// ManifoldManifold* manifold_refine(void* mem, ManifoldManifold* m, int refine); refine > 1
		...
		load("manifold_refine_to_length", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifold* manifold_refine_to_tolerance(void* mem, ManifoldManifold* m, double tolerance);
		load("manifold_refine_to_tolerance", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifold* manifold_smooth_by_normals(void* mem, ManifoldManifold* m,
		load("manifold_smooth_by_normals", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_INT);

		// ManifoldManifold* manifold_calculate_normals(void* mem, ManifoldManifold* m, int normal_idx, double min_sharp_angle);
		load("manifold_calculate_normals", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_DOUBLE);

	// ===== Split/Trim operations =====

		// ManifoldManifoldPair manifold_split(void* mem_first, void* mem_second, ManifoldManifold* a, ManifoldManifold* b);
		load("manifold_split", PAIR_LAYOUT, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS);

		// ManifoldManifold* manifold_trim_by_plane(void* mem, ManifoldManifold* m, double normal_x, double normal_y, double normal_z, double offset);
		load("manifold_trim_by_plane", ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE, ValueLayout.JAVA_DOUBLE);

		// ManifoldManifoldPair manifold_split_by_plane(void* mem_first, void* mem_second, ManifoldManifold* m, double normal_x, double normal_y, double normal_z, double offset);
	...
	// ===== Cleanup ======
		...
		// ManifoldMeshGL* manifold_meshgl(void* mem, float* vert_props, size_t n_verts, size_t n_props, uint32_t* tri_verts, size_t n_tris);
		load("manifold_meshgl",
			ValueLayout.ADDRESS,   // return: ManifoldMeshGL*
		...
		// ManifoldMeshGL64* manifold_meshgl64(void* mem, double* vert_props, size_t n_verts, size_t n_props, uint64_t* tri_verts, size_t n_tris);
		load("manifold_meshgl64",
			ValueLayout.ADDRESS,   // return: ManifoldMeshGL*
			ValueLayout.ADDRESS,   // arg1: void* mem
...
// ===== Primitives =====
	...
	// ManifoldManifold* manifold_cylinder(void* mem, double height, double radius_low, double radius_high, int circular_segments, int center);
	public MemorySegment cylinder(double height, double radiusLow, double radiusHigh, int segments, int center) throws Throwable {
		MemorySegment mem = (MemorySegment) functions.get("manifold_alloc_manifold").invoke();
		return (MemorySegment) functions.get("manifold_cylinder").invoke(mem, height, radiusLow, radiusHigh, segments, center);
	}

// ===== Transformations =====
	// 4x4 matrix transform (12 doubles = 3x4 affine matrix, last row implied 0,0,0,1)
	// Matrix layout: [x1 y1 z1]
	//                [x2 y2 z2]
	//                [x3 y3 z3]
	//                [x4 y4 z4]  <- translation
	// ManifoldManifold* manifold_transform(void* mem, ManifoldManifold* m, double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3, double z3, double x4, double y4, double z4);
	public MemorySegment transform(MemorySegment m,
		double x1, double y1, double z1,
		double x2, double y2, double z2,
        double x3, double y3, double z3,
        double x4, double y4, double z4) throws Throwable {
    
		MemorySegment mem = (MemorySegment) functions.get("manifold_alloc_manifold").invoke();
		return (MemorySegment) functions.get("manifold_transform").invoke(mem, m, x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4);
	}
...
//	}
	...
	public MemorySegment importMeshGL(float[] vertices, int[] triangles, long nVerts, long nTris) throws Throwable {
		...
		if (vertices.length < nVerts * 3 || triangles.length < nTris * 3) {
			throw new IllegalArgumentException("Array length mismatch: vertices=" + vertices.length + 
				" (expected " + (nVerts * 3) + "), triangles=" + triangles.length + " (expected " + (nTris * 3) + ")");
		}
	...
	public BoundingBox getJavaFXBounds(MemorySegment m) {
	...
	public double[] boxDimensions(MemorySegment box) throws Throwable {
		try (Arena arena = Arena.ofConfined()) {
			...
			return new double[] {
				dims.get(ValueLayout.JAVA_DOUBLE,  0), // width  (x)
	...
	public BoundingBox getBounds(MemorySegment m) throws Throwable {
		MemorySegment box = (MemorySegment) functions.get("manifold_alloc_box").invoke();
		try {
			functions.get("manifold_bounding_box").invoke(box, m);
			...
			try (Arena arena = Arena.ofConfined()) {
				...
				return new BoundingBox(0, 0, 0, w, h, d);
			}
		} finally {
			functions.get("manifold_delete_box").invoke(box);
		}
	...
	public long estimateMemoryBytes(MemorySegment m) throws Throwable {
		...
		// - Half-edge records: ~40 bytes each (twin/next/vert/face/edge as int64_t + padding)
